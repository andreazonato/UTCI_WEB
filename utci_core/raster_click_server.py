...
def _fetch_remote_tif(base: Path, folder: str, tif_name: str) -> str | None:
    ...
    urls = _remote_url_candidates(REMOTE_API_DATA_BASE, rel)
    for url in urls:
        try:
...
def _remote_url_candidates(base_url: str, rel: str) -> list[str]:
    quoted_rel = "/".join(urllib.parse.quote(p) for p in rel.split("/"))
    primary = base_url.rstrip("/") + "/" + quoted_rel
    out = [primary]

    # If configured with media.githubusercontent.com/media/... form, also try
    # the equivalent raw.githubusercontent.com/... URL because some blobs are
    # directly available there while media can return 404.
    parsed = urllib.parse.urlparse(base_url.rstrip("/"))
    if parsed.netloc == "media.githubusercontent.com":
        ...
        if len(parts) >= 4 and parts[0] == "media":
            ...
            if suffix:
                raw_base += "/" + suffix
            out.append(raw_base.rstrip("/") + "/" + quoted_rel)
    return out
